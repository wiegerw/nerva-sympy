...
LAYERS_FILE_DEFAULT = os.path.join(SIBLING_ROOT, 'nerva-torch', 'src', 'nerva_torch', 'layers.py')
TESTS_DIR_DEFAULT = os.path.join(SYMPY_ROOT, 'tests')


def main(argv: Optional[List[str]] = None) -> int:
    parser = argparse.ArgumentParser(description='Check consistency of nerva-torch layer equations with SymPy tests')
    parser.add_argument('-v', '--verbose', action='count', default=0, help='Increase verbosity (use -vv for debug)')
    parser.add_argument('--layers-file', default=LAYERS_FILE_DEFAULT, help='Path to layers.py to inspect (default: ../nerva-torch/src/nerva_torch/layers.py)')
    parser.add_argument('--tests-dir', default=TESTS_DIR_DEFAULT, help='Path to SymPy tests directory (default: ./tests)')
    args = parser.parse_args(argv)

    setup_logging(args.verbose)

    layers = extract_layer_equations_generic(args.layers_file, replace_matmul_with_mul=True)
