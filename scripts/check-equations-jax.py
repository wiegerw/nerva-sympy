...
LAYERS_FILE_DEFAULT = os.path.join(SIBLING_ROOT, 'nerva-jax', 'src', 'nerva_jax', 'layers.py')
TESTS_DIR_DEFAULT = os.path.join(SYMPY_ROOT, 'tests')


def main(argv: Optional[List[str]] = None) -> int:
    parser = argparse.ArgumentParser(description='Check consistency of nerva-jax layer equations with SymPy tests')
    parser.add_argument('-v', '--verbose', action='count', default=0, help='Increase verbosity (use -vv for debug)')
    parser.add_argument('--layers-file', default=LAYERS_FILE_DEFAULT, help='Path to layers.py to inspect (default: ../nerva-jax/src/nerva_jax/layers.py)')
