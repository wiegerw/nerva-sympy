...
def extract_layer_equations() -> Dict[str, Dict[str, List[str]]]:
...
def extract_all_test_sections() -> List[Dict[str, List[str]]]:
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    layers = extract_layer_equations()
    ...
    for layer_name, eqs in layers.items():
        match_info = None
        for b in blocks:
            if match_equations(eqs, b):
                meta = b.get('__meta__', {})
                match_info = (meta.get('file'), meta.get('index'))
                break
        if match_info:
            logging.info(f'Layer {layer_name}: matched test block in {match_info[0]} (block #{match_info[1]})')
        else:
